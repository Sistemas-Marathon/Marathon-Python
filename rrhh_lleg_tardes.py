...
def run():
    ...
    if st.button("📥 Consultar y descargar"):
        try:
            ...
            query = """
				SELECT DISTINCT 
				    PL.MPS_ETABLISSEMENT AS ETABLISSEMENT, 
				    ET_LIBELLE AS LIBELLE, 
				    PL.MPS_COMMERCIAL AS CODECOMMERCIAL, 
				    GCL_PRENOM + ' ' + GCL_LIBELLE AS COMMERCIAL, 
				    PL.MPS_DATEDEBPLAGE AS DATEHEUREPLANNING, 
				    PO.MPS_DATEDEBPLAGE AS DATEHEUREPOINTAGE, 
				...
				LEFT OUTER JOIN MPLAGESALARIE PO 
				    ON PL.MPS_IDPLAGE = PO.MPS_IDPLAGEPREV 
				LEFT OUTER JOIN COMMERCIAL 
				    ON PL.MPS_COMMERCIAL = GCL_COMMERCIAL 
				LEFT OUTER JOIN ETABLISS 
				    ON PL.MPS_ETABLISSEMENT = ET_ETABLISSEMENT 
				LEFT OUTER JOIN MTYPEPLAGE 
				    ON PL.MPS_TYPEPLAGE = MTY_TYPEPLAGE
				WHERE PO.MPS_DATEDEBPLAGE IS NOT NULL
				AND PL.MPS_DATEDEBPLAGE >= %s 
				AND PL.MPS_DATEDEBPLAGE <= %s
            ...
            LEFT OUTER JOIN MTYPEPLAGE ON p.MPS_TYPEPLAGE = MTY_TYPEPLAGE
