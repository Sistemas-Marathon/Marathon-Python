...
SELECT
  ...
  r.MPS_DUREEPLAGE AS Horas_Retenidas,
  t.MPS_DUREEPLAGE AS Horas_Realizadas,
  j1.CC_LIBELLE AS Justificacion1,
  j2.CC_LIBELLE AS Justificacion2,
  j3.CC_LIBELLE AS Justificacion3
...
LEFT JOIN MPLAGESALARIE t
  ON t.MPS_IDPLAGEPREV = p.MPS_IDPLAGE
  AND t.MPS_COMMERCIAL = p.MPS_COMMERCIAL
  AND t.MPS_MODEPLA = '003'
LEFT JOIN COMMERCIAL c
  ON p.MPS_COMMERCIAL = c.GCL_COMMERCIAL
LEFT JOIN MTYPEPLAGE g
  ON p.MPS_TYPEPLAGE = g.MTY_TYPEPLAGE
LEFT JOIN CHOIXCOD j1
  ON r.MPS_JUSTIF1 = j1.cc_code
  AND j1.cc_type='MJP'
LEFT JOIN CHOIXCOD j2
  ON r.MPS_JUSTIF2 = j2.cc_code
  AND j1.cc_type='MJP'
LEFT JOIN CHOIXCOD j3
  ON r.MPS_JUSTIF3 = j3.cc_code
  AND j1.cc_type='MJP'
WHERE p.MPS_MODEPLA = '001'
  AND CAST(p.MPS_DATEDEBPLAGE AS DATE) >= %s 
  AND CAST(p.MPS_DATEDEBPLAGE AS DATE) <= %s 
